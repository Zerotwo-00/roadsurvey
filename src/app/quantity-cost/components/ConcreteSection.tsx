...
export default function ConcreteSection({ inputMode, mapLength, onUpdate }: Props) {
  const [length, setLength] = useState<string>('');
  const [width, setWidth] = useState<string>('7.5');
  const [thickness, setThickness] = useState<string>('0.25');
  const [unitPrice, setUnitPrice] = useState<string>('180');
  ...
  useEffect(() => {
    if (cost !== null) {
      onUpdate({ id: 'concrete', name: 'الخرسانة', quantity: volume!, unit: 'م³', unitPrice: UP, total: cost });
    }
  }, [cost]);
    ...
    <div className="card">
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <input type="number" value={thickness} onChange={(e) => setThickness(e.target.value)} className="input-field" placeholder="مثال: 0.25" min="0" step="0.01" />
        </div>
        <div>
